...
class UpriserWidget {
  ...
  openModal(options = {}) {
    ...
    this.log("Opening modal:", { url, title });
    ...
    return { success: true, message: "Modal opened successfully" };
  }

  // Get client tools from config
  getClientTools() {
    const defaultTools = {
      open_modal: (params) => this.openModal(params)
    };
  ...
  setupClientTools() {
    ...
    const convaiCallHandler = (event) => {
      this.log("elevenlabs-convai:call event triggered, registering client tools");
      this.log("Event detail:", event.detail);
      
      if (event.detail && event.detail.config) {
        event.detail.config.clientTools = this.getClientTools();
        this.log("Client tools registered successfully:", Object.keys(this.getClientTools()));
        this.log("Full config after registration:", event.detail.config);
      } else {
        this.log("Event detail or config is missing:", event.detail);
      }
    };

    document.addEventListener('elevenlabs-convai:call', convaiCallHandler);

    const setClientToolsDirectly = () => {
      const widget = document.querySelector('elevenlabs-convai');
      if (widget) {
        widget.clientTools = this.getClientTools();
  ...
  // Update configuration including client tools
  updateConfig(newConfig) {
    this.config = { ...this.config, ...newConfig };
    this.log("Configuration updated:", newConfig);
    
    // If client tools changed, re-setup
    if (newConfig.clientTools) {
  ...
  setClientTools(clientTools) {
    this.config.clientTools = clientTools;
    this.setupClientTools();
    this.log("Client tools updated:", Object.keys(clientTools));
  }
...
(function() {
  ...
  function initializeWidget() {
    try {
      if (window.upriserWidgetInstance && window.upriserWidgetInstance.isInitialized) {
        console.log("[Upriser Widget] Already initialized");
        return window.upriserWidgetInstance;
      }

      const widgetInstance = new UpriserWidget(window.UPRISER_WIDGET_CONFIG);
      
      widgetInstance.init().catch(function(error) {
        console.error("[Upriser Widget] Initialization failed:", error);
      });

      window.upriserWidgetInstance = widgetInstance;
  ...
  if (typeof window !== 'undefined' && typeof document !== 'undefined') {
    ...
    window.initUpriserWidget = initializeWidget;
  }
})();